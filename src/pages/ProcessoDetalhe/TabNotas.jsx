...
export default function TabNotas({ notas, onAdd, onDel, onEdit, autorAtual }) {
  ...
  const iniciarEdicao = (n) => { setEditId(n.id); setEditTexto(n.texto); };
  const cancelarEdicao = () => { setEditId(null); setEditTexto(''); };
  const salvarEdicao = () => {
    if (!editTexto.trim()) return;
    onEdit(editId, editTexto.trim());
    cancelarEdicao();
  };
      ...
      {sorted.map(n => (
        <div key={n.id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '14px 16px', marginBottom: 8, display: 'flex', gap: 12 }}>
          <div style={{ flex: 1 }}>
            {editId === n.id ? (
              <>
                <textarea value={editTexto} onChange={e => setEditTexto(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && e.ctrlKey) salvarEdicao(); if (e.key === 'Escape') cancelarEdicao(); }} style={{ marginBottom: 8 }} autoFocus />
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
                  <button className="btn-secondary" onClick={cancelarEdicao}>Cancelar</button>
                  <button className="btn-primary" onClick={salvarEdicao} disabled={!editTexto.trim()}>Salvar</button>
                </div>
              </>
            ) : (
              <>
          ...
          {editId !== n.id && n.autor === autorAtual && (
            <div style={{ display: 'flex', gap: 4, alignSelf: 'flex-start' }}>
              <button className="btn-ghost" onClick={() => iniciarEdicao(n)} style={{ fontSize: 14 }}>✏️</button>
              <button className="btn-ghost" onClick={() => onDel(n.id)} style={{ fontSize: 14 }}>✕</button>
            </div>
          )}
        </div>
      ))}
